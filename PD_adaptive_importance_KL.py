...
#         distiled_model_dir = "distiled_model_new/kl_loss_baseline_iclr/{}/".format(game)
#         if not os.path.exists(distiled_model_dir):
#             os.makedirs(distiled_model_dir)
#         save_name = "{}{}_kl_{}.hdf5".format(distiled_model_dir,game, tau)
#         callbacks = [EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience = 50),ModelCheckpoint(save_name, save_best_only=True, save_weights_only=False)]
#         model.fit(x=X_train, y=Y_train, validation_data=(X_test, Y_test), batch_size=256, epochs=1001, callbacks= callbacks, use_multiprocessing=True, workers=8)
    ...
    for alpha in alpha_list:
        for entropy_temperature in entropy_temperature_list:
            ...
            distiled_model_dir = "distiled_model_new/adaptive_entropy_loss_tnnls_2/{}/".format(game)
            if not os.path.exists(distiled_model_dir):
                os.makedirs(distiled_model_dir)
            tau = 0.01
            save_name = "{}{}_kl_{}_alpha_{}_entropytemperature_{}.hdf5".format(distiled_model_dir,game, tau, alpha, entropy_temperature)
            ...
            history = model.fit(x=X_train, y=Y_train, validation_data=(X_test, Y_test), batch_size=128, epochs=1501, callbacks= callbacks, use_multiprocessing=True, workers=8)
            ############# save training history 
            history_dir = "{}/history/".format(distiled_model_dir)
            if not os.path.exists(history_dir):
                os.makedirs(history_dir)
            with open('{}training_history_kl_{}_alpha_{}_entropytemperature_{}.pkl'.format(history_dir, tau, alpha, entropy_temperature), 'wb') as file:
